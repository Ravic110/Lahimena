...
def _parse_city_days(raw: str) -> dict:
    ...
    for segment in re.split(r"[,;>/|\n]+", str(raw)):
        ...
        city_part = re.sub(
            r"\d+\s*(?:j|jour|jours)\b", "", city_part, flags=re.IGNORECASE
        )
...
class ClientHotelCotation(ClientCotationScreen):
    ...
    def _enregistrer_lignes(self, client, rows):
        return save_client_hotel_cotation_to_excel(client, rows)
    ...
    def _build_ui(self):
        ...
        for key, heading, width in self._COLS:
            ...
            anchor = (
                "e"
                if key
                in ("nuits", "nb_pax", "prix_unitaire", "depense", "marge", "total")
                else "w"
            )
            self._tree.column(key, width=width, anchor=anchor, stretch=False)

        vsb = ctk.CTkScrollbar(
            tree_outer, orientation="vertical", command=self._tree.yview
        )
        hsb = ctk.CTkScrollbar(
            tree_outer, orientation="horizontal", command=self._tree.xview
        )
        ...
        tk.Frame(totals_inner, width=1, bg="#C9DDE3").pack(
    ...
    def _open_row_dialog(self, row: dict, row_index):
        ...
        rp_vars: dict = {
            rk: {
                ...
                "price": tk.StringVar(
                    value=str(local_rp[rk]["price"]) if local_rp[rk]["price"] else ""
                ),
            }
            for _, rk, _ in _ROOM_TYPES
        }
        ...
        combo_group = ttk.Combobox(
            s1,
            textvariable=v_group,
            values=_GROUP_LABELS,
            font=ENTRY_FONT,
            width=18,
            state="readonly",
        )
        ...
        for row_i, (_, rk, lbl) in enumerate(_ROOM_TYPES):
            ...
            rp_vars[rk]["count"].trace_add(
                "write", lambda *_, r=rk: _update_subtotal(r)
            )
            rp_vars[rk]["price"].trace_add(
                "write", lambda *_, r=rk: _update_subtotal(r)
            )
        ...
        def _update_preview(*_):
            prix = sum(
                _to_int(rp_vars[rk]["count"].get())
                * _to_float(rp_vars[rk]["price"].get())
                for _, rk, _ in _ROOM_TYPES
            )
